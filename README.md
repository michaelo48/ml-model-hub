...
| worker | `SWEEP_INTERVAL_MS`, `SWEEP_GRACE_MS` | optional; defaults 21600000 (6 h) / 3600000 (1 h). Cadence of the orphaned-storage sweep (`0` disables) and the age below which an unreferenced object is left alone |
...
- Every `SWEEP_INTERVAL_MS` the worker lists both storage buckets and removes objects no row points at (`model_artifacts.storage_path`; `datasets.storage_path` plus the kept original upload). These come from best-effort deletes in the web app whose storage call failed, or from a crash between an upload and its row insert. Objects younger than `SWEEP_GRACE_MS` are never touched, because every upload-then-insert sequence in the codebase finishes in seconds and the row may simply not have landed yet. The live set is read in pages and checked against an exact row count; if they disagree (PostgREST silently caps a response at `db-max-rows`) the sweep aborts before removing anything. It runs in the background off the claim path, starts one jittered interval after boot rather than at boot, and is idempotent, so several worker instances sweeping is only redundant.
